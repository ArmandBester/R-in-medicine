...
- Very long variable names, discriptive, but difficult to work with
- The values contains confidance intervals in brackets, this will be difficult to work with as is
...
ARV_cov.df <- ARV_cov.df %>% 
  ...
  mutate(NumberInfected = str_replace_all(NumberInfected, "\\[.*?\\]", "")) %>% 
  mutate(NumberInfected = str_replace_all(NumberInfected, "<", "")) %>%
  mutate(NumberInfected = str_replace_all(NumberInfected, " ", "")) %>%
  mutate(PersCov = as.double(PersCov)) %>% 
  mutate(NumberOnARV = as.double(NumberOnARV)) %>% 
  mutate(NumberInfected = as.double(NumberInfected))
...
HIV_mort <- HIV_mort %>% 
  ...
  mutate(Deaths_2010 = str_replace_all(Deaths_2010, "\\[.*?\\]", "")) %>% 
  mutate(Deaths_2005 = str_replace_all(Deaths_2005, "\\[.*?\\]", "")) %>% 
  mutate(Deaths_2000 = str_replace_all(Deaths_2000, "\\[.*?\\]", "")) %>% 
  ...
  mutate(Deaths_2017 = as.double(Deaths_2017)) %>% 
  mutate(Deaths_2010 = as.double(Deaths_2010)) %>% 
  mutate(Deaths_2005 = as.double(Deaths_2005)) %>% 
  mutate(Deaths_2000 = as.double(Deaths_2000))
...
From the table above we can see that the mean for the dataset as a whole is about half of that during the early 2000s.  It would be interesting to plot this data, but as it will probably be to bussy as it is.  We can have a look at countries which had the most change rather.
...
HIV_mort %>% 
  ...
  na.omit() %>% 
  separate(Year, c("D", "Year"), sep = "_", convert = TRUE) %>% 
...
Remeber we mentioned that HAART was introduced in 2004 in South Africa, depicted here by the blue dotted line.  It is easy to appreciate the dramatic effect the introduction of ARVs had in South Africa.
