...
ARV_cov.df <- ARV_cov.df %>% 
  na_if("No data") %>% 
  mutate(PersCov = clean_deaths(PersCov),
         NumberInfected = clean_deaths(NumberInfected), 
         NumberOnARV = as.double(NumberOnARV))  %>% 
  filter(Country %in% ssa.list)
...
# 
# africa <- 
#  sf::st_read("Africa_SHP/Africa.shp") %>%
#  left_join(HIV_rate_2016, by = c(COUNTRY = "Country"))
# 
# africa %>%
#  ggplot() +
